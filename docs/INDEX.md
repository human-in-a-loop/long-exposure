...
| [`git-federation.md`](git-federation.md) | Many long-exposure instances on separate machines against one repo, and per-cycle commits for one. **Built:** operator identity, the read-only conflict radar, the git sync layer (per-cycle commits to a run branch, merge-in of the shared branch, push) with crash recovery verified by a real SIGKILL, and the per-module git policy the build enforces. **Not built:** the claims registry, deliberately. | Turning on per-cycle commits, or designing multi-operator work |
