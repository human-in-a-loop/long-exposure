...
| [`git-federation.md`](git-federation.md) | **Designed, not built.** Walkthrough of many long-exposure instances on separate machines committing to one repo: which of the three storage zones is shared, the two cycle-boundary git seams, how each file class converges (union-merged ledger, per-operator memoirs), git's push rejection as the lock and exactly where it stops working, and the ledger-identity gap to fix first | Designing or building the multi-operator sync layer |
