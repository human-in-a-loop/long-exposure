...
| [`worktrees-hooks-swarms-plan.md`](worktrees-hooks-swarms-plan.md) | Proposal and decision log: where git worktrees do and do not belong (nowhere, in the end), the Claude/Codex/Gemini lifecycle-hook intersection and the two hooks built from it, and swarms reframed twice — landing on a federation over one repo rather than a local swarm | Considering worktrees, hooks, or multi-operator / swarm scaling |
| [`git-federation.md`](git-federation.md) | Many long-exposure instances on separate machines against one repo, and per-cycle commits for one. **Built:** operator identity, the read-only conflict radar, the git sync layer (per-cycle commits to a run branch, a read-only mirror of peers' published work, push), the integrator that publishes each operator's work to the shared branch as a conflict-free projection, with crash recovery verified by a real SIGKILL, and the per-module git policy the build enforces. **Not built:** the claims registry, deliberately. | Turning on per-cycle commits, or designing multi-operator work |
...
| `git-federation.md` | ~400 | Contributor (design) |
