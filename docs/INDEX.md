...
| [`git-federation.md`](git-federation.md) | Many long-exposure instances on separate machines against one repo. **Built:** operator identity on the ledger, the read-only conflict radar, slice-name canonicalisation. **Not built:** the fetch/commit/push sync layer and the claims registry. Also: which of the three storage zones is shared, how each file class converges, and git's push rejection as the lock plus exactly where it stops working | Designing or building the multi-operator sync layer |
